...
public class Company
{
    ...
    // Использую LINQ для поиска отдела
    public Department FindDepartment(Func<Department, bool> criteria)
    {
        return Departments.FirstOrDefault(criteria);
    }

    // Использую LINQ для поиска сотрудника
    public Employee FindEmployee(Func<Employee, bool> criteria)
    {
        return Departments.SelectMany(d => d.Employees).FirstOrDefault(criteria);
    }
}
                ...
                case "employee":
                    ...
                    var employee = company.FindEmployee(
                        e => e.FullName.Equals(input, StringComparison.OrdinalIgnoreCase) ||
                             ...
                             (decimal.TryParse(input, NumberStyles.Any, CultureInfo.InvariantCulture, out salary) && e.Salary == salary)
                    );
                    if (employee != null)
                    {
